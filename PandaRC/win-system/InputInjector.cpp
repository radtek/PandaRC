...
{
	printf("Prepare to inject the key event: vkCode = %d, release = %d, extended = %d",
		(int)vkCode,
		(int)release,
		(int)extended);
	printf("The modifier states before: m_controlIsPressed = %d; m_menuIsPressed = %d; m_deleteIsPressed = %d; m_shiftIsPressed = %d; m_winIsPressed = %d;",
	...
	printf("The modifier states after: m_controlIsPressed = %d; m_menuIsPressed = %d; m_deleteIsPressed = %d; m_shiftIsPressed = %d; m_winIsPressed = %d;",
		(int)m_controlIsPressed,
		(int)m_menuIsPressed,
		(int)m_deleteIsPressed,
		(int)m_shiftIsPressed,
	...
	if (m_controlIsPressed && m_menuIsPressed && m_deleteIsPressed &&
		...
		if (m_ctrlAltDelEnabled) {
			printf("Try simulate the Ctrl+Alt+Del combination");
			Environment::simulateCtrlAltDel();
		}
		else {
			printf("The Ctrl+Alt+Del combination is disabled. Ignore the Del key pressing");
		}
	}
	else {
		INPUT keyEvent = { 0 };
...
{
	printf("Try insert a char event: char = %d, release = %d", (int)ch, (int)release);
	...
	try {
		...
		printf("Current keyboard layout = %x", (int)hklCurrent);
		vkKeyScanResult = searchVirtKey(ch, hklCurrent);
		printf("The virtual code scan result = %d", (int)vkKeyScanResult);
	}
	catch (...) {
		printf("Can't insert the char by simulating a key press event, therefore try insert it as an unicode symbol");
		if (ctrlOrAltPressed) {
			printf("Can't insert the char by an unicode symbol because a modifier is pressed");
			throw;
		}
		INPUT keyEvent = { 0 };
	...
	if ((ctrlPressNeeded || altPressNeeded) &&
		...
		printf("Received a control combination that we doesn't know how it can be made");
		return;
	}

	printf("Variable states before generate key events to get the char: controlSym = %d; resistantToCaps = %d; invariantToShift = %d; shiftedKey = %d; shiftPressNeeded = %d; shiftUpNeeded = %d; ctrlPressNeeded = %d; altPressNeeded = %d;",
		(int)controlSym,
		(int)resistantToCaps,
