...
Win32ScreenDriverBaseImpl::Win32ScreenDriverBaseImpl(UpdateKeeper *updateKeeper, UpdateListener *updateListener, LocalMutex *fbLocalMutex)
	: m_fbLocalMutex(fbLocalMutex),
	m_cursorPosDetector(updateKeeper, updateListener),
	m_curShapeDetector(updateKeeper, updateListener, &m_curShapeGrabber, fbLocalMutex)
{
}
...
{
  // Grabbing under the mutex avoid us from grab void cursor shape in time when the shape hides until grabs screen.
