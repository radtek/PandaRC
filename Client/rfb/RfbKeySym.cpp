...
{
		printf("processKeyEvent() function called: virtKey = %#4.4x, addKeyData = %#x", (unsigned int)virtKey, addKeyData);
		// Ignoring win key (when fullscreen mode is off).
		if (m_winKeyIgnore) {
			if (virtKey == VK_LWIN || virtKey == VK_RWIN) { // Ignoring the Win key
				printf("Ignoring the Win key event");
				return;
			}
		}

		bool down = (addKeyData & 0x80000000) == 0;
		printf("down = %u", (unsigned int)down);
		...
		printf("ctrl = %u, alt = %u, shift = %u, caps toggled = %u",
			...
			if (m_keyMap.virtualCodeToKeySym(&rfbSym, virtKey & 255)) {
				...
				printf("The key has been mapped to the %#4.4x rfb symbol", rfbSym);
					sendKeySymEvent(rfbSym, down);
			}
			else {
				WCHAR outBuff[20];
				...
				printf("ToUnicodeEx() return %d", count);
				...
				if (count == 1 && !m_allowProcessCharEvent || count > 1) {
					...
					if (ctrlAltPressed) {
						printf("Release the ctrl and alt modifiers before send the key event(s)");
							releaseModifiers();
					}
					for (int i = 0; i < count; i++) {
						if (onlyCtrlPressed && outBuff[i] < 32) {
							if (onlyCtrlPressed && outBuff[i] >= 1 && outBuff[i] <= 26 &&
								!shiftPressed) {
								printf("The %u char is a control symbol then it will be increased by 96 to %u",
									(unsigned int)outBuff[i], (unsigned int)outBuff[i] + 96);
									outBuff[i] += 96;
							}
							else {
								printf("The %u char is a control symbol then it will be increased by 64 to %u",
									(unsigned int)outBuff[i], (unsigned int)outBuff[i] + 64);
									outBuff[i] += 64;
							}
						}
						if (m_keyMap.unicodeCharToKeySym(outBuff[i], &rfbSym)) {
							printf("Sending the %#4.4x rfb symbol", rfbSym);
							sendKeySymEvent(rfbSym, down);
						}
						else {
							printf("Can't translate the %#4.4x unicode character to an  rfb symbol to send it", (unsigned int)outBuff[i]);
						}
					}
					if (ctrlAltPressed) {
						printf("Restore the ctrl and alt modifiers after send the key event(s)");
						restoreModifiers();
					}
				}
				else if (count == 0) {
					printf("Was get a not printable symbol then try get a printable with turned off the ctrl and alt modifiers");
						...
						count = ToUnicodeEx(virtKey, 0, withoutCtrlAltKbdState, outBuff,
							sizeof(outBuff) / sizeof(WCHAR),
							0, currentLayout);
						printf("ToUnicodeEx() without ctrl and alt return %d", count);
						if (count == 1) { // other case will be ignored
							if (m_keyMap.unicodeCharToKeySym(outBuff[0], &rfbSym)) {
								sendKeySymEvent(rfbSym, down);
							}
							else {
								printf("Can't translate the %#4.4x unicode character to an rfb symbol to send it", (unsigned int)outBuff[0]);
							}
						}
				}
				else if (count == -1 && down) {
					printf("Dead key pressed, wait for a char event");
	...
	unsigned int addKeyData)
{
	if (m_allowProcessCharEvent) {
		printf("processCharEvent() function called with alowed processing: charCode = %#4.4x, addKeyData = %#x", (unsigned int)charCode, addKeyData);
		m_allowProcessCharEvent = false;
		UINT32 rfbSym;
		if (m_keyMap.unicodeCharToKeySym(charCode, &rfbSym)) {
			...
			printf("Can't translate the %#4.4x unicode character to an rfb symbol to send it", (unsigned int)charCode);
		}
	}
}
...
{
	printf("Process focus restoration in the RfbKeySym class\n");
...
{
	printf(("Process focus loss in the RfbKeySym class"));
