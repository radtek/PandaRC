...
void UpdateFilter::filter(UpdateContainer *updateContainer)
{
  ...
  // Grabbing
 glog.debug(_T("grabbing region, %d rectangles"), (int)rects.size());
  ...
  ProcessorTimes pt2 = glog.checkPoint(_T("end of grabbing region"));
...
glog.debug(_T("Before grabbing region %f processor Mcycles, %f process time, %f kernel time, %f wall clock time "), 
    pt1.cycle / 1000000., pt1.process, pt1.kernel, pt1.wall.getTime());
glog.debug(_T("After grabbing region Mpoint grabbed: %f for %f processor Mcycles. %f ms"), 
    area, pt2.cycle/1000000., dt);
glog.debug(_T("After grabbing region %f process time, %f kernel time, %f wall clock time"), pt2.process, pt2.kernel, dt);

glog.debug(_T("end of grabbing region"));

  // Filtering
  pt1 = glog.checkPoint(_T("filtering changed"));
  ...
  pt2 = glog.checkPoint(_T("after filtering changed"));
  dt = pt2.wall.getTime(); // in milliseconds
glog.debug(_T("Before filtering changed %f processor Mcycles, %f process time, %f kernel time, %f wall clock time "),
    pt1.cycle / 1000000., pt1.process, pt1.kernel, pt1.wall.getTime());
glog.debug(_T("After filtering changed Mpoint filtered: %f for %f processor Mcycles. %f ms"),
    area, pt2.cycle / 1000000., dt);
glog.debug(_T("After filtering changed %f process time, %f kernel time, %f wall clock time"), pt2.process, pt2.kernel, dt);
}
