...
enet_host_service(ENetHost * host, ENetEvent * event, enet_uint32 timeout)
{
	...
	if (event != NULL)
	{
		...
		case -1:
#ifdef ENET_DEBUG
			sprintf(buffer, "Error dispatching incoming packets errno:%d", errno);
			perror(buffer);
#endif
	...
	do
	{
		...
		case -1:
#ifdef ENET_DEBUG
			sprintf(buffer, "Error sending outgoing packets errno:%d", errno);
			perror(buffer);
#endif
		...
		case -1:
#ifdef ENET_DEBUG
			sprintf(buffer, "Error receiving incoming packets errno:%d", errno);
			perror(buffer);
#endif
