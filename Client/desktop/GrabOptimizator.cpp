...
bool GrabOptimizator::grab(const Region *grabRegion, ScreenDriver *grabber)
{
  ...
  if (getOptimizationAvailable(grabber)) {
    ...
    if (boundsRectT <= estimatedFragTime) {
	  ...
	  glog.debug(_T("Bounds rectangle grab has been preferred:")
     ...
     glog.debug(_T("Fragment grab has been preferred:")
...
void GrabOptimizator::refreshStatistic(ScreenDriver *grabber)
{
  if (m_timer.isElapsed() && getOptimizationAvailable(grabber)) {
   glog.debug(_T("The log statistics before refreshing:"));
    logStatistic();
    ...
    m_timer.reset();
   glog.debug(_T("The log statistics after refreshing:"));
    logStatistic();
  }
}
...
{
 glog.debug(_T("GrabOptimizator::m_wholeS : %d"),
             m_wholeS);
 ...
 glog.debug(_T("GrabOptimizator::m_wholeT average: %.2f;")
 ...
 glog.debug(_T("GrabOptimizator::m_g average: %.2f;")
